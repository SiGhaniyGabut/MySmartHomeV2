...
class AsyncWebsocketClient:
    def __init__(self, ms_delay_for_read: int = 5):
        ...
        self._lock_for_open = asyncio.Lock()
        self.sock = None
    ...
    def urlparse(self, uri):
        ...
        if match:
            ...
            if protocol not in ['ws', 'wss']: raise ValueError('Scheme {} is invalid'.format(protocol))

            if port is None: port = (80, 443)[protocol == 'wss']
    ...
    async def a_readline(self):
        ...
        while line is None:
            ...
            await asyncio.sleep_ms(self.delay_read)
    ...
    async def handshake(self, uri, headers=[]):
        ...
        if self.uri.protocol == 'wss': self.sock = ssl.wrap_socket(self.sock, server_hostname=self.uri.hostname)
        ...
        for key, value in headers: send_header(b'%s: %s', key, value)
        ...
        if not header.startswith(b'HTTP/1.1 101 '): return False
    ...
    async def read_frame(self, max_size=None):
        ...
        # Mask is 4 bytes
        if mask: mask_bits = await self.a_read(4)
    ...
    async def recv(self):
        while await self.open():
            ...
            # except (ValueError, EOFError) as ex:
                ...
                print(f'Error reading frame: {ex}')
                await self.open(False)
                return
            ...
            if opcode == OP_TEXT:
                ...
                try:
                    ...
                    print(f'Error sending PONG frame: {ex}')
